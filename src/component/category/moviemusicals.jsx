...
const Moviemusicals = () => {
  ...
  useEffect(() => {
    const apiUrl = "https://cdn.pixabay.com/video/2015/08/08/125-135736646_tiny.mp4";
    fetch("https://jsonplaceholder.typicode.com/photos")
      .then((response) => response.json())
      .then((data) => {
        const transformedData = data.slice(0, 20).map((item) => ({
          id: item.id,
          url: item.url,
    ...
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mt-5">
        {channels.map((channel) => (
          ...
          >
            <Link to={`/moviemusicalsDetails/${channel.id}`}>
              <div
